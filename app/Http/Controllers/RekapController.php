...
class RekapController extends Controller
{
    ...
    public function bunga_investor(Request $request)
    {
        ...
        $bulan = $request->bulan ?? date('m');
        $tahun = $request->tahun ?? date('Y');
        $dataTahun = BungaInvestor::selectRaw('YEAR(created_at) tahun')->where('kas_ppn', $data['kas_ppn'])->groupBy('tahun')->get();

        $data = BungaInvestor::whereMonth('created_at', $bulan)->where('kas_ppn', $data['kas_ppn'])->whereYear('created_at', $tahun)->get();
     ...
     public function cost_operational(Request $request)
    {
        ...
        $data = $kas->costOperational($bulan, $tahun);
