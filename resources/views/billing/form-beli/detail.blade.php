...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12 text-center">
            <h1><u>FORM BELI<br>{{$b->barang_unit?->nama}}
            <br>{{$b->kas_ppn_text}} <br>{{$b->sistem_pembayaran_text}}</u></h1>
        </div>
    </div>
    @include('swal')
     @include('billing.form-beli.modal-keranjang')
            ...
            <table class="table">
                <tr class="text-center">
                    ...
                    <td class="text-center align-middle"><a href="{{route('billing')}}"><img
                            ...
                            Billing</a></td>
                    @endif
                        <td class="text-center align-middle"><a href="{{route('billing.form-beli')}}"><img
                                src="{{asset('images/back.svg')}}" alt="dashboard" width="30"> Kembali</a></td>
                </tr>
            </table>
        </div>
    </div>
...
<div class="container mt-3 table-responsive ">
    ...
    <div class="row my-3">
        <div class="col-md-6">
            <div class="row px-3">
                <a href="{{route('billing.form-beli.detail.preview', $b->id)}}" class="btn btn-success @if ($keranjang->count() == 0) disabled
                @endif" role="button"><i class="fa fa-shopping-cart"></i> Keranjang {{$keranjang->count() == 0 ? '' :
                    '('.$keranjang->count().')'}}</a>
            </div>
        </div>
        <div class="col-md-6">
            <form action="{{route('billing.form-beli.detail.empty', $b->id)}}" method="post"
                id="keranjangEmpty">
                @csrf
                <div class="row px-3">
...
<script>
    ...
    $(document).ready(function() {
        var table = $('#stok-datatable').DataTable({
            ...
            serverSide: true,
            ajax: {
            url: "{{ route('billing.form-beli.detail.data', $b->id) }}",
        ...
        $('#stok-datatable tbody').on('click', 'button.btn-modal-trigger', function() {
            ...
            if (harga > 0) {
                // Jika qty > 0 (mode edit), isi modal dengan data yang ada
                const formatter = new Intl.NumberFormat('id-ID');
                const formattedHarga = formatter.format(harga);
