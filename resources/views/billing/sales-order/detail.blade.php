...
<div class="container">
    ...
    <div class="row mb-3 d-flex">
        <div class="col-md-6">
            <a href="{{route('billing.sales-order', ['kas_ppn'=> $order->kas_ppn])}}" class="btn btn-secondary"><i class="fa fa-arrow-left"></i>
                Kembali</a>
        </div>
    </div>
    <div class="row">
        <form action="{{route('billing.sales-order.detail.update', ['order' => $order->id])}}" method="post" id="storeForm">
            @csrf
            <div class="card">
...
<script>
    ...
    function deleteKeranjang(id)
    {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{route('billing.sales-order.detail.delete', ':id')}}'.replace(':id', id),
                    type: 'POST',
                    data: {
                        id: id,
                        _token: '{{ csrf_token() }}'
